...
public class ChatController {
    ...
    public String chatPost(@RequestParam("likeId") Long likeId, HttpSession session, Model model) {
        ...
        String other = like.getMember().getNickname();
        model.addAttribute("username", memberService.getLoggedInMember(session).getNickname());
        model.addAttribute("otherName", other); // 채팅방 식별 정보
        return "chat";
    }
    ...
    @SendTo("/topic/{roomId}")  // 특정 채팅방의 구독자들에게 전달할 경로
    public ChatMessage sendMessage(ChatMessage message) {
        message.setSendTime(LocalDateTime.now());
        return message; // 메시지를 그대로 반환하여 채팅방에 전달
    }
    ...
    public ChatMessage join(ChatMessage chatMessage) {
        chatMessage.setContent(chatMessage.getSender() + " 님이 입장하였습니다.");
